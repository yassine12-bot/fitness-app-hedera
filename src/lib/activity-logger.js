...
class ActivityLogger {
  ...
  /**
   * Logger la complétion d'un challenge
   */
  async logChallengeCompleted(userId, challengeId, challengeName, reward, hederaTxId = null) {
    return this.log(userId, 'challenge_completed', {
      challengeId,
      challengeName,
      reward,
      hederaTxId
  ...
  /**
   * Logger un transfert de tokens Hedera
   */
  async logTokenTransfer(userId, amount, recipientId, hederaTxId = null) {
    return this.log(userId, 'token_transfer', {
      amount,
      recipientId,
      hederaTxId
    });
  ...
  async logAdminAction(adminId, action, targetUserId = null, details = {}) {
    return this.log(adminId, 'admin_action', {
      action,
      targetUserId,
      ...details
    });
