...
router.post('/', authMiddleware, upload.single('media'), async (req, res) => {
    try {
        ...
        // ✅ NOUVEAU : Mettre à jour les challenges sociaux
        try {
            await axios.post(
                `http://localhost:${process.env.PORT || 3000}/api/challenges/update-social`,
                {},
                { headers: { Authorization: req.headers.authorization } }
            );
            console.log('📊 Challenge social mis à jour (+1 post)');
        } catch (challengeError) {
            console.log('⚠️ Pas de challenges sociaux actifs ou erreur:', challengeError.message);
        }
