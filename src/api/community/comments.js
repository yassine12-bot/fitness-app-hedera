...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        if (isPositive) {
            ...
            // Récupérer les infos user pour Hedera
            const user = await db.get(
                'SELECT id, hederaAccountId, fitBalance FROM users WHERE id = ?',
                [req.user.id]
            );
            ...
            if (user && user.hederaAccountId && process.env.FIT_TOKEN_ID) {
                try {
                    ...
                    if (hederaTransferResult.success) {
                        console.log(`✅ ${fitReward} FIT tokens Hedera transférés à ${user.hederaAccountId} (commentaire positif)`);
                    }
                } catch (hederaError) {
                    console.error('⚠️ Erreur transfert Hedera pour commentaire:', hederaError.message);
                }
            }
            
            // 📝 NOUVEAU: Logger l'événement dans le registry Hedera
            try {
                console.log('🔍 [DEBUG] Début du logging positive_comment_reward dans Hedera...');
                
                const userForLog = await db.get('SELECT hederaAccountId FROM users WHERE id = ?', [req.user.id]);
                const userId = userForLog?.hederaAccountId || `user_${req.user.id}`;
                ...
                const logResult = await activityLogger.log(
                    userId, 
                    'positive_comment_reward', 
                    {
                        postId: postId,
                ...
                if (logResult && logResult.success) {
                    console.log(`✅ Événement "positive_comment_reward" enregistré dans le registry Hedera (seq: ${logResult.sequenceNumber})`);
                } else {
                    console.error('⚠️ Logging comment échoué:', logResult?.error || 'Raison inconnue');
                }
            } catch (logError) {
                console.error('❌ ERREUR logging comment registry:', logError.message);
