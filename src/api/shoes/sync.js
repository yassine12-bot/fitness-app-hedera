...
router.post('/sync', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      const challengeResult = await axios.post(
  `http://localhost:${process.env.PORT || 3000}/api/challenges/update-progress`,
  { steps },
  { headers: { Authorization: req.headers.authorization } }
);
if (challengeResult.data.completedChallenges?.length > 0) {
  console.log(`🎉 ${challengeResult.data.completedChallenges.length} challenge(s) auto-validé(s)!`);
} else {
  console.log(`📊 Progression challenges mise à jour (+${steps} pas)`);
}
    } catch (challengeError) {
      console.log('⚠️ Pas de challenges actifs ou erreur update:', challengeError.message);
    }
...
/**
 * POST /api/shoes/manual-steps
 * Simulateur manuel de pas (par clics frontend)
 */
router.post('/manual-steps', authMiddleware, async (req, res) => {
  try {
    const { steps = 50 } = req.body; // 50 pas par défaut par clic
    ...
    // Enregistrer comme workout
    await db.run(`
      INSERT INTO workouts (userId, steps, distance, calories, workoutDate)
      VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
    `, [req.user.id, steps, steps * 0.0008, steps * 0.04]); // Estimations
    ...
    console.error('❌ Erreur pas manuels:', error);
    res.status(500).json({
      success: false,
      message: 'Erreur ajout pas',
      error: error.message
    });
  }
});
